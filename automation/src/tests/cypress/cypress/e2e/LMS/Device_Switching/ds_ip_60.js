...
import { ip60_online } from '../../../support/functions/ip60_online'
/*----------------------------------------------------------------------------------------------------------------
Script: HH1
Open Defects:
...
describe('LMS Online Survey IP60', function () {

  it('P60 Device Switching Start', function() {
    //initialise sts
    func.check_sts();
    ...
    ip60_online("Screen", "No", "Start");

    cy.request('http://nfs-server.c.ons-tiss-eng.internal:9191/sts/ADD?FILENAME=dev_switch_ip60_ts5_sand_.csv&LINE=' + uac +'&ADDMODE=LAST&UNIQUE=TRUE').then((response) => {
      expect(response).property('status').to.equal(200);

    });


    cy.request('http://nfs-server.c.ons-tiss-eng.internal:9191/sts/SAVE?FILENAME=dev_switch_ip60_ts5_sand_.csv').then((response) => {
...
it('P60 Device Switching Finish', function() {
  func.mob_sizer();
  //initialise sts
  func.check_sts();
  ...
  //GET GUID
  cy.request('http://nfs-server.c.ons-tiss-eng.internal:9191/sts/READ?READ_MODE=FIRST&KEEP=FALSE&FILENAME=dev_switch_ip60_ts5_sand_.csv').then((response) => {
    expect(response).property('status').to.equal(200);
    cy.log(response.body);
    guid = func.extract_guid(response.body);
    cy.log(guid);
