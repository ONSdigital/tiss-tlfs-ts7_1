...
function ip13_online(type = "Screen", full = "Yes", partial) {
  ...
  influx.wait_start_timer(1000);
    ...
    var list = ["I deal with my own tax or National Insurance"];
    ...
    cy.contains('.text-container', 'In your second job, who deals with your tax or National Insurance');
    cy.get('.text-container').contains('My accountant deals with my tax or National Insurance').click();
    func.save_cont();
...
if ( full == "Yes" || partial == "Finish") {
    ...
    cy.get('[data-fieldname="QIndiv[1].QEmploy.POT1_Hours"][role="textbox"]').clear().type('10');
    cy.get('[data-fieldname="QIndiv[1].QEmploy.POT1_Mins"][role="textbox"]').clear().type('0');

    cy.contains('.text-container', 'In your second job, how many hours of paid overtime did you actually work in that week?');
    cy.get('[data-fieldname="QIndiv[1].QEmploy.POT2_Hours"][role="textbox"]').clear().type('3');
    cy.get('[data-fieldname="QIndiv[1].QEmploy.POT2_Mins"][role="textbox"]').clear().type('0');
    func.save_cont();
    ...
    cy.get('[data-fieldname="QIndiv[1].QEmploy.UsHr1_Hours"][role="textbox"]').clear().type('40');
    cy.get('[data-fieldname="QIndiv[1].QEmploy.UsHr1_Mins"][role="textbox"]').clear().type('0');
    //func.save_cont();

    cy.contains('.text-container', 'how many hours did you actually work in your main job, excluding overtime?');
    cy.get('[data-fieldname="QIndiv[1].QEmploy.ActHr1_Hours"][role="textbox"]').clear().type('50');
    cy.get('[data-fieldname="QIndiv[1].QEmploy.ActHr1_Mins"][role="textbox"]').clear().type('0');
    func.save_cont();
    ...
    cy.get('[data-fieldname="QIndiv[1].QEmploy.UsHr2_Hours"][role="textbox"]').clear().type('10');
    cy.get('[data-fieldname="QIndiv[1].QEmploy.UsHr2_Mins"][role="textbox"]').clear().type('0');
    //func.save_cont();

    cy.contains('.text-container', 'how many hours did you actually work in your second job, excluding overtime?');
    cy.get('[data-fieldname="QIndiv[1].QEmploy.ActHr2_Hours"][role="textbox"]').clear().type('14');
    cy.get('[data-fieldname="QIndiv[1].QEmploy.ActHr2_Mins"][role="textbox"]').clear().type('0');
    func.save_cont();
    ...
    cy.get('[data-fieldname="QIndiv[1].QEmploy.SOC_2020_2_Pt1"][role="textbox"]').clear('textarea');
    cy.get('[data-fieldname="QIndiv[1].QEmploy.SOC_2020_2_Pt1"][role="textbox"]').type('Sales Executive');
    func.save_cont();

    cy.contains('.text-container', 'What do you mainly do in that job');
    cy.get('[data-fieldname="QIndiv[1].QEmploy.SOC_2020_2_Pt2"][role="textbox"]').clear('textarea');
    cy.get('[data-fieldname="QIndiv[1].QEmploy.SOC_2020_2_Pt2"][role="textbox"]').type('Sell Computers');
    func.save_cont();

    cy.contains('.text-container', 'At your second job, what does the firm or organisation mainly make or do?')
    cy.get('[data-fieldname="QIndiv[1].QEmploy.SIC_2020_2_Emp"][role="textbox"]').clear('textarea');
    cy.get('[data-fieldname="QIndiv[1].QEmploy.SIC_2020_2_Emp"][role="textbox"]').type('Computer sales');
    func.save_cont();
    ...
    cy.contains('.text-container', 'Just to check, is £35000.00 the full-time pay for your second job?').then(() => {
    ...
    //LMS-2535 - blank page appears with just save and continue on Mobile only
    // if(type == "Mobile") {
    //   func.save_cont(3000);
    // }
